// main.js ── 式変形マスター（複数入力フィールド対応・汎用版）
'use strict';
(function(){

/* ===== localStorage ===== */
const STORE_KEY = 'oniripi_2gen_v1';
const mem = {};
const store = {
...
// 1次式 y=ax+b を解析して {a,b} を返す（記述式の式全体採点用）
function safeNum(str){
  str=String(str).replace(/^\+/,'');
  if(/^[-+()\d/.]+$/.test(str) && str!==''){ try{ return Function('return ('+str+')')(); }catch(e){ return NaN; } }
  return NaN;
}
function parseLinear(latex){
  let s=String(latex||'');
  s=s.replace(/\\dfrac/g,'\\frac').replace(/\\left|\\right|\\!|\\,|\\;|\\cdot|\\mleft|\\mright/g,'');
  s=s.replace(/\s+/g,'').replace(/[−ー–—]/g,'-').replace(/[＝]/g,'=').replace(/[＋]/g,'+');
  const eq=s.indexOf('=');
  if(eq>=0) s=s.slice(eq+1);
  s=s.replace(/\\frac\{(-?\d+)\}\{(-?\d+)\}/g,'($1/$2)');
  let a, rest;
  const m=s.match(/([+-]?)(\((?:-?\d+\/-?\d+)\)|\d+\.?\d*)?x/);
  if(m){
    const sign=m[1]==='-'?-1:1;
    const coef=m[2];
    a=(coef===undefined||coef==='')?sign*1:sign*safeNum(coef);
    rest=s.replace(m[0],'');
  }else{ a=0; rest=s; }
  rest=rest.replace(/^\+/,'');
  const b=(rest===''||rest==='+'||rest==='-')?0:safeNum(rest);
  return {a,b};
}
...
function buildFields(q){
  ...
  if(q.mode==='equation'){
    const wrap=document.createElement('span'); wrap.className='mf-inline eq-mode';
    const mf=document.createElement('math-field'); mf.setAttribute('virtual-keyboard-mode','off');
...
function showResult(){
  ...
  const msgs=pass?['🏆 おにリピ クリア！','🔥 完ぺき！式変形マスター！','⭐ すごい！この調子！']
